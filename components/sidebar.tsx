...
export default function Sidebar() {
  ...
  return (
    <>
      {/* ── Left sidebar ─────────────────────────────── */}
      <aside className="fixed left-0 top-0 h-screen w-14 z-50 flex flex-col items-center py-8 gap-4 hidden sm:flex">
        {/* Monogram */}
        <button
          onClick={() => scrollTo("hero")}
          className="font-mono font-bold text-sm text-accent tracking-widest"
        ...
        {/* Vertical label */}
        <span
          className="font-mono text-[10px] text-ink-3 tracking-widest uppercase"
      ...
      <nav className="fixed right-5 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3 hidden sm:flex">
        {SECTIONS.map((s) => (
          ...
          >
            {/* Label on hover */}
            <span className="font-mono text-[10px] text-accent opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest">
              {s}
            </span>
            <span
